"""
Training script for Random Forest Classifier (5 classes).
...
from training.classification.decision_tree_random_forest import RandomForestClassifierFromScratch
...
MAX_DEPTH = 5          
CRITERION = 'gini'
SUBSAMPLE_RATIO = 1  # Use 100% of training data per fold
...
def main(data_path):
    ...
    print(f"\nTraining Random Forest ({N_TREES} trees) with {n_splits}-fold CV...")
    print(f"Using subsample ratio: {SUBSAMPLE_RATIO} for faster training")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_scaled, y_resampled_array)):
        ...
        # Subsample training data for this fold to speed up
        n_subsample = int(len(train_idx) * SUBSAMPLE_RATIO)
        subsample_idx = np.random.choice(train_idx, size=n_subsample, replace=False)
        ...
        rf.fit(train_scaled[subsample_idx], y_resampled_array[subsample_idx].reshape(-1, 1))
        
        # Get predictions
        val_pred = rf.predict(train_scaled[val_idx])
        test_pred_fold = rf.predict(test_scaled)
        
        # Convert to one-hot for averaging
        for i, pred in enumerate(val_pred):
            oof[val_idx[i], int(pred)] += 1
        for i, pred in enumerate(test_pred_fold):
            preds_test[i, int(pred)] += 1 / n_splits
    ...
    plt.tight_layout()
    cm_path = 'output/plots/random_forest_confusion_matrix.png'
    ...
    ax.set_title('Per-Class Performance Metrics (Random Forest)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    metrics_path = 'output/plots/random_forest_class_metrics.png'
    ...
    plt.tight_layout()
    dist_path = 'output/plots/random_forest_class_distribution.png'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Random Forest classifier (5-class)')
