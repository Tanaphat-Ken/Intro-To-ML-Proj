...
def main(data_path, target_column=None, tune=False):
    ...
    # Diagnostic output
    print(f"\n[Diagnostics]")
    print(f"  Initial loss: {lasso_model.loss_history[0]:.6f}")
    print(f"  Final loss: {lasso_model.loss_history[-1]:.6f}")
    print(f"  Iterations completed: {len(lasso_model.loss_history)}/{N_ITERATIONS}")
    if len(lasso_model.loss_history) > 10:
        recent_decrease = lasso_model.loss_history[-10] - lasso_model.loss_history[-1]
    ...
    print(f"Model improvement: {(1 - lasso_results['test_mse']/baseline_mse)*100:.2f}% better than baseline")
    ...
    coeffs_dict = {name: coef for name, coef in zip(feature_names, lasso_model.coefficients)}
    ...
    # L1 sparsity - features with non-zero coefficients
    selected_features = lasso_model.get_selected_features(feature_names, threshold=1e-5)
    print(f"\nL1 Sparsity: Selected {len(selected_features)} out of {len(feature_names)} features")

    # Count near-zero coefficients
    near_zero = sum(1 for coef in lasso_model.coefficients if abs(coef) < 1e-3)
    print(f"Features with near-zero coefficients (<0.001): {near_zero}/{len(feature_names)}")
    ...
    # Save model and plot
    print("\nSaving model...")
    lasso_model.save_weights(str(MODELS_DIR / f'lasso_regression_alpha_{best_alpha}.pkl'))

    print("\nPlotting loss curve...")
    plot_path = str(PLOTS_DIR / f'lasso_regression_alpha_{best_alpha}_loss.png')
    lasso_model.plot_loss(save_path=plot_path)
